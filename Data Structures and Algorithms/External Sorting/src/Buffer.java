...
/**
 * @author Hulya Dogan and Chris Lursen
 * @version 11/10/2020
 * 
 *          Buffer is to help interface with the byte
...
public class Buffer {
    ...
    /**
     * @param record
     *            is a new record to add at the current
     *            position if there is space
     */
    public void write(byte[] record) {
        if (!full()) {
            System.arraycopy(record, 0, byteArray, position, RECORD_SIZE);
    ...
    public byte[] read() {
        ...
        position += RECORD_SIZE;
        return Arrays.copyOfRange(byteArray, position - RECORD_SIZE, position);
    }
